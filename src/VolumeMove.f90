...
!
  ...
  Real (wp)  ::  lnvn, xi,  v_new, factor
  Real (wp)  ::   side_old, factor_inv
  Real (wp), external :: f
...
!
  ...
  If (Exp(-(deltaEt+pres*(v_new-v0))+dble(natoms)*log(v_new/v0)) .Gt. xi) Then
         nvaccept = nvaccept + 1
         E_sr = E_sr_new
...
!
  ...
  Real (wp)  ::  xi,  rcpcut2_old, factor, rcpcut_old
