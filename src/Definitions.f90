...
!
  use set_precision
  implicit none
  Integer, Parameter :: ndim=3, natmx=10
  Integer :: nsp, natoms
  Integer, Allocatable ::ntype(:), iatype(:)
...
!
  ...
  Integer :: nb, nstep, ntrial, naccept, naver, nequil, npgr, ntraj
  Real(wp) :: temp, kT, s_cput, e_cput
  Real(wp) :: rdmax(1:ndim)
  character :: ensemble*3, initcf*3
End Module rundata
...
!
  Use set_precision
  Integer, Parameter :: nde=50000
  Real(wp) :: Etotal, E_sr, E_coulomb, E_fourier, Eref, deltaE, deltagr
