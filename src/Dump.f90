Subroutine Cierra(clean)
    ...
    Use linkcell
    Use interp
    Use writecfg, only : writecfg_dlp, writecfg_lmp
    ...
    dfname = adjustl(trim(res_dir))//"/dump"//dia(1:8)//hora(1:4)//".dmp"
    call random_seed(size=length)
    call random_seed(get=seed(1:length))
    open (1000,file=dfname,form="unformatted",access="stream")
    ...
    write(1000) Etotal, E_sr, E_coulomb, E_fourier, Evdw, deltagr
    write(1000) Etav, E_sav, E_lrav, E_vdwav
    write(1000) temp, kT, pres,  deltagr, rdmax, vdmax
    ...
    if (clean == 0) then
        ...
        if (initcf == "dlp") then
           call writecfg_dlp()
        elseif (initcf == "lmp") then
           call writecfg_lmp(istep)
        else
           write(*,'("*** Error: ",a3," trajectory format not implemented")')
        endif
...
1100    Format("   #################################################################"/ &
            & "   #                                                               #"/ &
            & "   #    dumping restart file and last conf for Program gpMC        #"/ &
            & "   #",19x,"  Time : ",a10,4x," Date : ",a10,"   #"/,  &
            & "   #",19x,"  CPU usage =",f15.3," mins.          #"/ &
            & "   #                                                               #"/ &
            & "   #################################################################")
...
Subroutine Load
    ...
    Implicit None
    Character ::  Dia*10, Hora*10, Bhora*10, BDia*10, dfname*30
    open (1000,file=adjustl(trim(res_dir))//"/restart.dmp",form="unformatted",access="stream")
    call random_seed
    call random_seed(size=length)
    ...
    Read(1000) Etotal, E_sr, E_coulomb, E_fourier,  Evdw, deltagr
    Read(1000) Etav, E_sav, E_lrav, E_vdwav
    Read(1000) temp, kT, pres, deltagr, rdmax, vdmax
